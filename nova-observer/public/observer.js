  ...
  if (runInFlight) {
    if (message) {
      pendingSubmissionPrompts.push((sourceIdentity || Object.keys(metadata).length)
        ? { text: message, sourceIdentity, metadata }
        : message);
...
voiceToggleBtn.onclick = () => {
  unlockSpeech();
  const toggleEvent = new CustomEvent("observer:voice:toggle-requested", {
    cancelable: true,
    detail: {
      enable: !voiceListeningEnabled,
  ...
  if (voiceListeningEnabled) {
    ...
    updateVoiceUi();
    window.dispatchEvent(new CustomEvent("observer:voice:state-changed", {
      detail: { listeningEnabled: false, wakeActive: voiceWakeActive, provider: "browser-speech-recognition", at: Date.now() }
    }));
    return;
  }
  ...
  window.dispatchEvent(new CustomEvent("observer:voice:state-changed", {
    detail: { listeningEnabled: true, wakeActive: voiceWakeActive, provider: "browser-speech-recognition", at: Date.now() }
  }));
};
...
resetTaskReshapeIssuesBtn.onclick = async () => {
  ...
  try {
    const adminFetch = typeof observerApp.adminFetch === "function"
      ? observerApp.adminFetch.bind(observerApp)
      : fetch;
    const r = await adminFetch("/api/tasks/reshape-issues/reset", {
      method: "POST",
      headers: { "content-type": "application/json" }
    });
    const j = await r.json();
