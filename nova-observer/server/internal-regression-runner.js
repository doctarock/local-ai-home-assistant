export function createInternalRegressionRunner({
  ...
  return async function runInternalRegressionCase(testCase) {
    ...
    if (mode === "project_work_failed_attempt_count") {
      const actualCount = Number(await countFailedProjectWorkAttempts(testCase.projectWorkKey, testCase.taskCache || {}));
      const failures = [];
      if (actualCount !== Number(testCase.expectedCount || 0)) {
        failures.push(`Expected failed project work attempts ${Number(testCase.expectedCount || 0)}, got ${actualCount}.`);
      }
      return {
        passed: failures.length === 0,
        failures,
    ...
    if (mode === "project_retry_threshold") {
      ...
      try {
        const message = buildCapabilityMismatchRetryMessage({
          message: "Advance the project Example in /home/nova/.observer-sandbox/workspace/Example.\nThis is a focused project work package, not a full project sweep.",
          projectPath: "/home/nova/.observer-sandbox/workspace/Example"
